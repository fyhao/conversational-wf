		...
		if(steps && steps.length) {
			...
			var checkNext = function() {
				...
				if(curStep < steps.length) {
					//console.log('modFlow.processStep ' + (1+curStep) + '/' + steps.length);
					processStep(steps[curStep], checkNext);
				}
				else {
					if(done.length == 1) {
						//console.log('modFlow.done 2');
						...
						//console.log('modFlow.done');
						process.nextTick(done);
					}
				}
			}
			//console.log('modFlow.checkNext');
			process.nextTick(checkNext);
		}
		else {
			//console.log('modFlow.done 1');
			process.nextTick(done);
		}
	}
